...
---
#### На схеме пунктирной линией обозначены данные, безопасность которых должна нас беспокоить
![Alt text](clickhouse.png)
### Установка:

- определим конфигурацию кластера  
...
- Автоматизация: Право на выполнение clickhouse-backup без запроса пароля позволяет автоматизировать процесс резервного копирования.  
    Это особенно полезно для заданий cron, которые должны выполняться без вмешательства человека.  
- Безопасность: sudo позволяет ограничить команды, которые может выполнять backupadmin. В данном случае,  
    backupadmin может выполнять только /bin/clickhouse-backup, что снижает риск злоупотребления правами.  
- Отслеживание: Использование sudo позволяет отслеживать все действия, которые выполняются с привилегиями суперпользователя. 
    Это может быть полезно для аудита и отладки.  
- Принцип наименьшей привилегии: backupadmin получает только те привилегии, которые необходимы для выполнения своей работы. 
    Это соответствует принципу наименьшей привилегии, который является основой хорошей практики безопасности.  
```sh
